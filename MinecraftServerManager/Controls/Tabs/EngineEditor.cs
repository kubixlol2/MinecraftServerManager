...
namespace MinecraftServerManager.Controls
{
    public partial class EngineEditor : UserControl, IStyleableTab
    {
        ...
        public new void Load(Tabs tabs)
        {
            ...
            this.FullRefresh();
            tabs.AddTab("Organizuj silniki", this);
        }
        ...
        private void FullRefresh()
        {
            ...
            foreach (TreeNode tn in this.engineTree.Nodes)
            {
                ...
                foreach (string version in Utils.Main.EngineVersions)
                {
                    ...
                    {
                        ntn.Text = version + " (Pobrano)";
                        ntn.Checked = true;
                    }
                    else
                    {
                        ntn.Text = version + " (Nie pobrano)";
                        allChecked = false;
                    }
                    ntn.Tag = version;
                    tn.Nodes.Add(ntn);
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            ...
            DownloadIndex = 0;
            DialogResult dr = MessageBox.Show("Zostanie pobrane " + DownloadQueue.Count + " silników i usunięte " + DeleteQueue.Count + " silników. \nCzy chcesz kontynuować?", "Informacja", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (dr == DialogResult.Yes)
            {
                ...
                foreach (DeleteUnit du in DeleteQueue)
                {
                    ...
                    tn.Text = tn.Text.Replace("Pobrano", "Nie pobrano");
                }
                DeleteQueue.Clear();
                Download();
            }
        ...
        private void DownloadProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            ...
            saveButton.Text = "Pobieranie silnika " + DownloadQueue[DownloadIndex].Readable + ": " + megaBytesReceived + " / " + megaBytesTotal;
        ...
        private void DownloadProgressCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            TreeNode tn = this.engineTree.Nodes[du.EngineIndex].Nodes[du.VersionIndex];
            tn.Text = tn.Text.Replace("Nie pobrano", "Pobrano");
            DownloadIndex++;
            Download();
        }
